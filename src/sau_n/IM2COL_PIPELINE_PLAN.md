...
- 有效空间 mask 必须是从 lane 0 开始的 canonical prefix。
...
### 7.3 S2：组装和 FIFO push

S2 将完整的 16-lane activation、空间 mask 和标签写入深度 4 FIFO。S2 必须支持
下游反压并保持 payload 稳定，直到 push handshake 成功。
...
无 `input_fire` 时，PE 累加器、`accepted_k`、activation、weight 和标签都不前进，
也不再抛出“输入流包含气泡”的异常。
...
`W=6, stride=2` 用于覆盖 `W<16` 多行 packing 下的纵向重复 bank group。因为本计划
不支持非对称 stride，这两个测试都使用：
...
S1 lane_done/request banks/conflict round
...
- 新增定向气泡测试，确认累加器和索引保持；保留 strict 模式气泡抛异常测试。
...
- 运行旧模型已有 RTL strict regression，确认连续输入路径未变化。
...
| packing 冲突 | W6, stride2 | W<16 同 bank 不同 row |
