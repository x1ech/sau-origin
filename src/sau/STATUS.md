...
- Current stage: Tasks 3 and 4 complete; Task 5 is next.
...
| 5. Implement token buffers and array timing | Next | Add bounded token storage and configurable fill/II/in-flight behavior. |
...
python3 util/style.py --modifications \
    src/sau/SConscript \
    src/sau/address_generator.hh \
    src/sau/address_generator.cc \
    src/sau/address_generator.test.cc

scons build/ALL/sau/address_generator.test.opt \
      build/ALL/sau/command.test.opt -j4
...
- The current model has no array pipeline, memory port, retry/backpressure,
...
1. Implement Task 5 using TDD:
   - bounded `TokenBuffer`;
   - `ArrayPipeline` fill latency;
   - initiation interval;
   - maximum in-flight tokens; and
   - token conservation tests.
2. Continue with the SimObject and timing-memory integration in Tasks 6-9.
